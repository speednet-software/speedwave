...
- **SSOT alignment:** `crates/speedwave-runtime/src/consts.rs::NODEJS_SUBDIR` (`nodejs`) must stay aligned with the literal `nodejs` in (a) the bundled NSIS sweep (`windows/sweep.ps1` filters on `$INSTDIR\nodejs\`, embedded into `installer-hooks.nsh` by `scripts/generate-installer-nsh.sh`), (b) `crates/speedwave-runtime/src/bundle.rs` `BundledAssetSpec` paths (`nodejs/bin/node`, `nodejs/node.exe`). Renaming the bundled-Node subdir = updating all three locations in the same commit. Pinned by `nodejs_subdir_appears_in_installer_hooks` test in `consts.rs`. See ADR-048 §"PRE-INSTALL orphan worker sweep".
