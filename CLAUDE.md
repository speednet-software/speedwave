...
- **SSOT alignment:** `crates/speedwave-runtime/src/tz.rs::detect_host_timezone()` (host TZ → `TZ` env injected into every service by `compose::inject_host_timezone`) must stay aligned with the `tzdata` package install in three Containerfiles: (a) `containers/Containerfile.claude` (apt), (b) `containers/mcp-servers/Containerfile.mcp-base` (apk), (c) `mcp-servers/hub/Containerfile` (apk). Adding a new base image = installing `tzdata` in the same commit; otherwise `TZ` resolves to a numeric offset and Claude Code limit timestamps stay wrong.
