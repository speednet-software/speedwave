...
@test "msvc-env.sh exports VULKAN_SDK so a Git Bash build can find the SDK" {
    local block
    block="$(awk '/\$sh \+= "# Generated by/,/Set-Content -Path \$envShWin/' "$SETUP_SCRIPT")"
    echo "$block" | grep -q "export VULKAN_SDK="
}

@test "the VULKAN_SDK export reads the machine env rather than a hardcoded path" {
    grep -q "GetEnvironmentVariable('VULKAN_SDK', 'Machine')" "$SETUP_SCRIPT"
    ! grep -qE "export VULKAN_SDK='C:" "$SETUP_SCRIPT"
}

@test "a missing machine VULKAN_SDK is reported instead of writing an empty export" {
    local guard
    guard="$(awk '/\$vulkanSdk = \[Environment\]/,/^        }$/' "$SETUP_SCRIPT")"
    echo "$guard" | grep -q 'IsNullOrWhiteSpace($vulkanSdk)'
    echo "$guard" | grep -q 'failedItems'
}
