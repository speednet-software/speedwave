...
- **List issues**: `redmine.listIssueIds` with `project_id` and `status: "open"` returns ids only; read each with `redmine.getIssueFull` and keep the issues whose subject starts with the role you want. Unprefixed subjects are the untriaged bucket. `redmine.searchIssueIds` with the role word narrows a large project first (ids only as well).
...
- **Frontier query**: `redmine.listIssueIds` with `status: "open"` scoped to the project returns ids only; read each with `redmine.getIssueFull` (`include: ["relations"]`), keep the children of the map (their `parent` is the map), drop any with an open blocker or an assignee; first in map order wins.
