---
name: slack
description: Use Slack integration to read and send Slack messages as the person who authorized Slack in Speedwave, under their own name and avatar; there is no bot and no service account, so every message you post is indistinguishable from one they typed themselves and you see only the channels they belong to. Covers listing those channels, reading or exporting channel history and threads (with pagination), reading and sending direct messages (1:1 DMs and group DMs), reading text files shared in channels (markdown, code, logs), posting messages, replying inside a thread, and finding workspace users by name or e-mail. Use whenever the user asks about Slack, for example "what's new on #channel", "summarise this week's discussion including threads", "read the file Paweł shared", "post an update to the team", "reply in this thread", "write a DM to Anna", "what did Marek send me in our DM", "find the message where...", etc. Use even when you think you know the answer, channel content is dynamic, only the live API reflects current messages. Do not use for Slack administration (creating channels, inviting members, workspace settings), or messaging platforms other than Slack.
...
**Never send without explicit confirmation, no exceptions.** Every message, channel post, 1:1 DM, group DM, is sent as the user, under their name and avatar, and cannot be unsent by you. Before ANY send, restate the exact destination (channel name, or the recipient's full name for a DM, or the full member list for a group DM) and the verbatim message text, then wait for explicit go-ahead in the current turn. The global write-confirmation rule in `claude-resources/CLAUDE.md` applies with extra weight here: a general instruction like "let them know" is not approval of final text, show the draft first. Never post drafts, partial summaries, or anything the user has not approved verbatim. Opening a DM conversation (`openDirectMessage`) is silent on the other side and needs no confirmation; sending into it always does. Confirm the placement too: a reply going into a thread versus at channel level is a different message to a different audience, and `reply_broadcast: true` widens a thread reply to the whole channel, so it needs its own go-ahead.
...
**Replying inside a thread.** `sendChannel({ channel, message, thread_ts })` posts the reply into that thread; omit `thread_ts` and it lands at channel level as a new message, detached from the discussion. Copy `thread_ts` verbatim from `getChannelMessages`/`getThreadMessages`, exactly as for reading: the worker rejects a reformatted or rounded value, because Slack itself accepts a malformed `thread_ts` silently and posts to the channel instead of the thread. Add `reply_broadcast: true` to also surface the reply in the channel, for the rare case where the outcome of a thread matters to everyone; it requires `thread_ts` and must be a real boolean, not the string `"false"`.
...
**Permission errors usually mean an outdated grant.** `missing_scope` / "Permission denied" on ANY operation (files, DMs, history) most often means the user signed in before that capability shipped, so their grant lacks the newer permissions. Suggest **Re-authorise Slack in Speedwave Desktop (Integrations, Slack)**: one sign-in refreshes the full permission set. Do not retry the call until they have.
...
- **Reply in a DM thread**: the same `sendChannel({ channel: "D…", message, thread_ts })` path works in DMs; nothing DM-specific is needed.
