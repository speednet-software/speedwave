...
- **Target versions (a milestone or a planning week) go through `fixed_version_id`.** Resolve a version name to its ID with `listVersions`, then pass it to `createIssue`/`updateIssue` or filter `listIssueIds` by it; `null` in `updateIssue` clears the version. Redmine omits `fixed_version` from an issue that has none.
- **Undeclared parameters are rejected, not ignored.** A call with a parameter the tool's schema does not list fails with an error naming it and the accepted ones, and nothing reaches Redmine: fix the name and retry instead of treating the call as done.
