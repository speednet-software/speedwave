...
- **Bad arguments are rejected, not ignored.** An undeclared parameter, a missing required one, or a numeric ID that is not a positive integer fails with an error naming it, and nothing reaches Redmine: fix the call and retry instead of treating it as done. Always call tools with an object of named parameters, e.g. `resolveUser({ identifier: 'me' })`.
