...
- **ETag CAS on uploads only.** `uploadFile` supports `expectedEtag` (If-Match) and `createOnly` (If-None-Match: \*), fetch the current state via `listFileIds`/`getFileFull` first when overwrite safety matters. `updatePage` has NO ETag or conditional-update support: concurrent edits to the same page can silently clobber each other, and there is no way to detect this from the tool. Mitigate it by asking: a page other people are likely to be viewing or editing (a team wiki, a shared status page) warrants warning the user their update could overwrite someone else's concurrent edit before you push it, not just proceeding silently.
- **User-scoped queries ("my items", "items assigned to me").** The tool description names the current-user tool to call first; then filter `listItems` against a list-specific author/assignee column (inspect `getList`'s column schema first, column names vary per list). `listItems`/`getItem` do not expose creator/modifier metadata by default, only user-defined list fields.
