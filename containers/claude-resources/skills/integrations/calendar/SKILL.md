...
- **`calendar_id` accepts ID or name; the event `id` does not.** `calendar_id` (used to filter/target a calendar) resolves by ID first, then falls back to a name match. `createEvent` refuses a name shared by several calendars: call `os.listCalendars()` and pass the id. The `id` passed to `getEvent`/`updateEvent`/`deleteEvent`, by contrast, must be the exact id string from a list/get/create call: a calendar or event display name is not accepted there.
- **Date/time format.** Pass `"2026-06-15T09:30:00"` for 09:30 in the user's local time and a bare `"2026-06-15"` for local midnight (all-day events: `all_day: true` with date-only `start`/`end`, `end` being the day after the last day, so `"2026-06-15"` to `"2026-06-16"` is June 15 only); an offset or `Z` is converted. Responses give `start`/`end` as UTC instants ending in `Z`: convert them to the user's local time before presenting. Relative phrases ("tomorrow") must be resolved to an absolute date first. `listEvents` defaults to now → +7 days when no range is given; always pass an explicit range for non-trivial queries.
