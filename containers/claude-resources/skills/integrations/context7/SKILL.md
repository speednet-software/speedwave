...
- Refactoring, writing scripts from scratch, debugging business logic, code review, or general programming concepts: answer directly, no tool call needed.
- The user's own repository configuration, pipeline files, or CI/CD setup: that is github or gitlab, not Context7. Context7 covers the tool's own documentation (e.g. "how does GitLab CI syntax work"), not "why is our pipeline failing" (the configured GitLab instance's actual pipeline state).
