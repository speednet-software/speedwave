...
mod tests {
    ...
    fn bundle_scripts_env_knobs_are_in_sync() {
        // The bats suite isolates itself from the real tree through these knobs; a knob read in one script
        // only, or bypassed by its hard-coded default path, leaves that platform's bundle unisolated.
        const KNOBS: [&str; 4] = [
            "BUNDLE_DEST",
        ...
        // Path operands of a PowerShell line: each quoted string whole, bare words split on whitespace
        // and quotes, stopping at a `#` that starts a word.
        ...
        let ps1 = std::fs::read_to_string(repo_root.join("scripts/bundle-build-context.ps1"))
            .expect("bundle-build-context.ps1 should exist");
        // Both scripts comment with `#`; a knob named only in a comment is not a read. Reads match with
        // whitespace removed, so reformatting one does not hide it.
        ...
        for knob in KNOBS {
            ...
            // `VAR="${KNOB:-$REPO_ROOT/<default>}"` ↔ `$var = if ($env:KNOB) { ... } else { '<default>' }`.
            let default = sh_line
