...
static HOST_ADDRESSING: std::sync::RwLock<Option<HostAddressing>> =
...
#[cfg(test)]
pub fn set_host_addressing_computer_for_test(
    computer: std::sync::Arc<dyn HostAddressingComputer>,
) {
...
mod host_addressing_impls {
    ...
    mod parse_tests {
        ...
        fn extracts_ipv4_from_default_route_line() {
            ...
            assert_eq!(parse_default_route_gateway(out), Some("172.24.48.1".to_string()));
        }

        #[test]
        fn returns_none_when_no_default_route() {
            assert_eq!(parse_default_route_gateway("10.4.1.0/24 dev br-xxx\n"), None);
        }
        ...
        fn ignores_non_default_lines() {
            ...
            assert_eq!(parse_default_route_gateway(out), Some("172.30.96.1".to_string()));
        }
    }
}
...
"#;
    ...
    fn ensure_host_gateway_extra_host_replaces_existing_canonical_entry() {
        ...
        assert_eq!(
            entries[0],
            format!("{}:{}", consts::HOST_GATEWAY_ALIAS, host_gateway_ip().expect("test"))
        );
    }
