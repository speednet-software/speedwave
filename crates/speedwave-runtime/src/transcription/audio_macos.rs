...
mod tests {
    ...
    fn frame_header(stream: u32, nframes: u32, offset_ns: u64) -> Vec<u8> {
        let mut out = Vec::new();
    ...
    fn frame(stream: u32, offset_ns: u64, samples: &[f32]) -> Vec<u8> {
        let mut out = frame_header(stream, samples.len() as u32, offset_ns);
        for &s in samples {
    ...
    fn read_frame_rejects_an_implausible_frame() {
        let mut r = raw_reader_over(&frame_header(0, 1_000_000_000, 0));
        let err = read_settled(&mut r).unwrap_err();
        assert!(matches!(err, CaptureError::Failed(_)));
        assert!(r.done);
        let mut hdr2 = frame_header(0, 1, 48 * 3600 * 1_000_000_000);
    ...
    fn read_frame_errors_on_a_truncated_payload() {
        let mut bytes = frame_header(0, 4, 0);
