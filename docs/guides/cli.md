...
- **`speedwave update [--project <name>]`** — rebuilds the built-in images whose build inputs changed (per-image hash tags, ADR-071), syncs claude-resources, and recreates containers with the current bundle manifest. Targets the active project by default; `--project <name>` overrides it.
...
- Compose rendering resolves built-in images as `speedwave-*:<per-image-hash>` — a 16-char hash of each image's declared build inputs (see [ADR-071](../adr/ADR-071-per-image-build-input-hash-tags.md))
