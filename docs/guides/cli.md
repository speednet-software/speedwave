...
- **`speedwave login [--project <name>]`** — runs `compose_up`, then exec's into the Claude container and runs `claude auth login`, which starts the Anthropic OAuth flow automatically (browser sign-in, paste-back code if the localhost callback can't reach the host). Any non-Anthropic provider env is unset first so OAuth runs against Anthropic regardless of the active provider. Claude Code stores credentials inside the container at `~/.claude/.credentials.json`, persisted on the host via the per-project `CLAUDE_HOME` bind-mount. If `--project <name>` is omitted, uses the active project. See [ADR-052](../adr/ADR-052-anthropic-oauth-login-flow.md).
