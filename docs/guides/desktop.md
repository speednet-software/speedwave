...
**Recordings & retention.** Each session stores `audio.wav` + `transcript.json` under `~/.speedwave/transcripts/<id>/` (0600). Audio is always kept; there is no auto-cleanup — the session list shows each recording's audio size and offers "discard audio" (keeps the transcript, frees disk, makes re-transcription impossible) and "delete transcript" (removes the whole directory). If audio was discarded, the offline pass can't run.
