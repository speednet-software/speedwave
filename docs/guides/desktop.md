...
**What runs locally vs. over the network.** Audio inference (Whisper transcription) runs locally — no audio leaves the machine for inference. The one-time model download uses the network (≈1.6–3.1 GiB depending on hardware). "Send to chat" drops the rendered transcript text into the chat (sent to your configured LLM provider) and opens the chat tab. The UI states this on every relevant surface.

**Workflow.** Pick a language (Polish or English — never auto-detect; forced beats auto on Whisper), an audio source, and (for mic/mixed sources) a specific microphone → **Start recording**. The source picker offers three options and defaults to **"Whole meeting (system audio + your microphone)"** — system loopback (what you hear: the other participants) and your mic, mixed into one stream; this is the headphones-on-a-call case. The other two are system-wide audio only, or just a microphone. The live transcript fills in as you speak, with a short delay by design: a sentence appears once it is a few seconds behind the newest audio (ADR-056 Amendment 6), so shown text never flickers or disappears. **Stop** triggers a higher-quality offline pass: it loads the recorded WAV, re-transcribes the whole recording (better cross-utterance context), and swaps the result in. Then **Send to chat** sends the transcript markdown, topped with a summarization instruction in the session language (summary, decisions, action items), into the chat and opens the chat tab (a confirm dialog runs first, the text leaves the machine). One practical limit of the single mixed stream: when loud system audio and your voice overlap continuously, the transcriber follows the dominant speaker; in a turn-based meeting both sides come through.
...
**Permissions (macOS).** When you press Record on a source that includes the microphone, Speedwave itself asks for **Microphone** access first (an in-app `AVCaptureDevice.requestAccess` call, ADR-056 Amendment 5): the consent prompt only fires from the app process, a headless helper cannot show it. The grant lands under `pl.speedwave.desktop` and the bundled `audio-capture-cli` records under it; the prompt text comes from the main app's `NSMicrophoneUsageDescription`. When the source includes system audio, macOS also asks for **System Audio Recording** access. That prompt has no public trigger, so `audio-capture-cli` requests it via the private `TCCAccessRequest(kTCCServiceAudioCapture)` API (the same approach AudioCap / AudioTee use; it works on a notarized `.dmg`), see ADR-056 decision 3. If you denied the mic prompt earlier, pressing Record deep-links you straight to System Settings → Privacy & Security → Microphone; re-enable Speedwave there. To reset the prompts entirely run `tccutil reset Microphone pl.speedwave.desktop` (and `tccutil reset AudioCapture pl.speedwave.desktop`). If the system-audio recording is silent while audio is clearly playing, the permission is most likely off; the UI surfaces this and links to the Settings pane. Settings → Meeting transcription also carries a **Permissions** block: the microphone consent state (with a request button before the first prompt) and deep-links to both privacy panes.

**Per-OS requirements.** macOS 14.4+ (CoreAudio process taps). Windows uses WASAPI system-wide loopback; when a headset holds the _communications_ device role (where Teams/Slack play call audio), the mixed capture records that endpoint too, so the call is not lost to the speakers-only loopback. Windows shows no per-app microphone prompt for desktop apps: if the mic side records nothing, check Settings → Privacy & security → Microphone ("Let desktop apps access your microphone").

**Capture health warnings.** The recording view shows an amber banner when something degrades mid-recording instead of failing silently: _no system audio captured_ (usually the System Audio Recording permission — the banner links to the Settings pane), _microphone stopped delivering_ or _system audio stopped delivering_ (the recording continues with the healthy side; a device switch, e.g. AirPods connecting when you join a call, is restarted automatically on macOS). A model download in progress survives switching tabs — Settings re-attaches to it when you come back.
