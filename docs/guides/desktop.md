...
**Per-OS requirements.** macOS 14.4+ (CoreAudio process taps). Windows uses WASAPI system-wide loopback; when a headset holds the _communications_ device role (where Teams/Slack play call audio), the mixed capture records that endpoint too, so the call is not lost to the speakers-only loopback.
