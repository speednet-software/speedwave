...
**Permissions (macOS).** The first time you record, macOS prompts for **Microphone** and **Audio Recording** access (the bundled `audio-capture-cli` carries `NSMicrophoneUsageDescription` / `NSAudioCaptureUsageDescription`). If you denied them, open System Settings → Privacy & Security → Microphone / Audio Recording and re-enable Speedwave; to reset the prompts entirely run `tccutil reset Microphone pl.speedwave.desktop.audio-capture` (and `tccutil reset AudioCapture pl.speedwave.desktop.audio-capture`).
