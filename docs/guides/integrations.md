...
- _Reactive_ — a 401 from Graph triggers the same `oauth.refresh()` under a mutex, then retries the original request. Both paths run through the shared `authedRequest` helper in `mcp-shared`, so the refresh-retry logic is identical across every OAuth integration.
