...
The entrypoint gates declarations with the same filters as symlinks (core: always; integrations: `ENABLED_SERVICES`, with OS sub-services additionally requiring `os` enabled and the sub-service absent from `DISABLED_OS_SERVICES`; plugins: `SPEEDWAVE_PLUGINS`), concatenates them per event into the `hooks` key of `~/.claude/settings.json`, and tracks what it injected in `~/.claude/.speedwave-managed-hooks` so a disabled source's hooks are unregistered on the next start. Hooks you or your team add to any settings scope are never touched. A malformed `hooks.json` is skipped with a warning. See [ADR-078](../adr/ADR-078-claude-hook-registration.md) for the design and security analysis.
