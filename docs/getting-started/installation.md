...
Speedwave forces WSL2 **mirrored** networking for VPN compatibility. Under mirrored mode its host workers (`node.exe`, the desktop app) bind loopback (`127.0.0.1`); containers reach them through a guest-side `socat` relay on a private guest-local address, so nothing is exposed to the LAN (ADR-079). Under legacy **NAT** networking (auto-detected at runtime), the host workers instead bind the WSL vEthernet adapter IP, which needs a Hyper-V firewall rule to be reachable across the WSL VM boundary. In **both** modes, host Windows Defender Firewall allow rules for the binaries suppress the per-app "allow access to the network" prompt. These rules require administrator rights: the MSI creates them at install time (no prompt). The per-user `.exe` installer runs without admin, so Speedwave creates them on first launch instead — you will see **one** UAC elevation prompt per app launch until the rules exist. Accept it once and no further prompts appear (the rules persist). If you decline, the app still works but Windows may show its own per-app firewall prompts; Speedwave does not remember the decline, so it offers the UAC prompt again the next time you start the app (at most once per launch — never repeatedly within a session). Removing Speedwave deletes the rules.
