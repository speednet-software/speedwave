...
The v0.11+ installer automatically releases the bundled `node.exe` (host MCP workers) before overwriting it, so upgrading while Speedwave is running no longer fails with "Error opening file for writing". If you are still on v0.10.x and hit that error, close Speedwave from the system tray, end any leftover `Node.js JavaScript Runtime` processes in Task Manager, and re-run the installer — once you are on v0.11+ this is handled automatically.
