...
- The Claude container `depends_on` the proxy with `condition: service_started`, so the proxy is created first on `compose up`. It is start-ordering only — the proxy has no healthcheck (unimplemented in the pinned nerdctl), and Claude Code only sends `/v1/messages` on the first user prompt, by which point the scratch-binary proxy is listening (ADR-073)
