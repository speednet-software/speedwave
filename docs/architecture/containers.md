...
- After each reconcile, `prune_orphan_current_bundle_images` force-removes worker tags of the current bundle that are no longer in `enabled_images(active)`. The same prune also runs at the end of every successful `restart_integration_containers`, scoped to the union across all projects, so disabling an integration in any project frees the worker image's disk as soon as no project still uses it.
