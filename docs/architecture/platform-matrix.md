...
- **Update — orphan `node.exe` handling:** Host-side MCP workers (`mcp-os`, `host_exec`, `oauth`) run as `node.exe` processes launched from the bundled `$INSTDIR\nodejs\node.exe`. Two layers prevent a stale worker from blocking the next installer with "Error opening file for writing": (1) a Windows Job Object with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` is attached to every worker at spawn (`host_mcp_process::job_object`), so a parent crash kills its children automatically; (2) the NSIS `NSIS_HOOK_PREINSTALL` hook tree-kills `Speedwave.exe` and runs a PowerShell sweep filtered by process Path against `$INSTDIR\nodejs\*`, releasing any orphan that survived. The sweep is strictly scoped — `node.exe` instances from other locations (VS Code, nvm, user shells) are never touched. Layer (1) only protects upgrades from v0.11+ (the version that ships the Job Object); layer (2) covers upgrades from any prior version.
