...
## PII policy: tokenization before the model sees it

The tokenization step above is driven by a per-project **policy**, not a fixed pattern list: `@speedwave/policy-engine` (`mcp-servers/policies/`) resolves a named template (or a custom selection) plus additive custom patterns and sensitive-key deltas into one `ResolvedPolicy`, and `mcp-hub` compiles that policy once at startup into the exact regex set `tokenizePII`/`detokenizePII` run against. Values that match are replaced with `[TYPE:TOKEN]` placeholders before the tool result reaches Claude; only the token, never the original value, crosses into the model's context. See [ADR-079](../adr/ADR-079-policy-engine-pii-tokenization.md) for the full design.

**Delivery: a mounted, read-only, digest-versioned file, not env vars or a live channel.** The host resolves the policy and writes it to `<data_dir>/policies/<project>/policy.json` (0600, atomic write), and the compose renderer mounts that directory read-only into `mcp-hub` at `/policy`, with `POLICY_FILE=/policy/policy.json` and a `POLICY_DIGEST` (sha256 of the file) that forces a container recreate when the policy content changes: the same mounted-file-plus-digest pattern used for the proxy's per-project config. A policy edit takes effect on the hub's next start, not by live-reloading a running container.

**Fail-closed, in the direction that filters more.** `POLICY_FILE` unset or the file absent resolves to the compiled-in default (all 8 built-in categories on), the safe fallback, because more filtering is the harmless failure direction for a PII guard. A file that is _present_ but unreadable, not valid JSON, or carries an unsupported schema version makes the hub log and exit non-zero at startup: a rendering bug can never leave a project silently running with a weaker policy than it asked for. The one deliberately soft failure is a single custom pattern that fails its ReDoS lint at load time: that one pattern is skipped with an error log while the rest of the policy (built-in categories, other custom patterns) still applies, because a user-typo'd regex should not take down PII protection for the whole project.

**Enforced by `SecurityCheck`, not just convention.** The `HubPolicyMount` rule makes the mount presence-mandatory on every rendered compose: exactly one `<policies dir>:/policy:ro` volume on `mcp-hub`, no other volume targeting the `/policy` area, and `POLICY_FILE` pinned to exactly `/policy/policy.json`. A compose that dropped the mount, weakened it to `:rw`, or repointed the env fails `SecurityCheck::run()` before `compose_up`, on every start path (CLI, Desktop, update, rollback).

**Custom patterns are user-supplied regex, so they go through a two-layer ReDoS gate** before they can tokenize anything: length and shape limits, no backreferences or lookaround, no nested unbounded quantifiers (the `(a+)+` class), no empty-string match. The Rust save-gate (backed by the non-backtracking `regex` crate plus an explicit nested-quantifier scan) is authoritative for what gets persisted; the TS lint at load time is the defensive second layer, using a different regex flavor with the accepted JS-vs-RE2 gap this repo already documented for plugin field patterns (ADR-015).
...
| `HubPolicyMount`              | mcp-hub                   | Exactly one canonical `<policies dir>:/policy:ro` volume, no other volume targeting `/policy`, and `POLICY_FILE` pinned to `/policy/policy.json` ([ADR-079](../adr/ADR-079-policy-engine-pii-tokenization.md)) |
