...
| `SpeedwaveProxyVolumes`       | speedwave-proxy           | Mount profile is exactly `/config:ro` + `tokens/<project>/llm:/tokens:ro` + `/usage:rw`, and no `network_mode` ([ADR-073](../adr/ADR-073-embedded-per-project-litellm-proxy.md)) |
