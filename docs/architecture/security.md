...
- **Per-recipe confirmation.** Each recipe declares `confirm: ask` (prompt every time — the default), `session` (prompt once per argv/cwd per app session), or `always` (never). `always` is only offered when editing a recipe, behind a second warning, and is **rejected** for recipes that look state-changing (a DB client; `docker compose up/down/exec/rm/prune`; a migration tool — `host_exec::is_state_changing_recipe`). A `docker`/`docker-compose`/`podman` *lifecycle* recipe is stricter still — forced to `confirm: "ask"` (not even `session`), because it is `docker run` with whatever mounts/privileges a Claude-editable compose file declares (≈ host root) and so must re-prompt every time. On a frontend non-answer the worker **fails closed** (the command doesn't start). The confirmation cache key is the resolved `argv` (as a `Vec`, not a delimited join — a join isn't injective over Claude-supplied param values) plus the cwd label plus a hash of the recipe's config — so a different param value, a different cwd, or an edited recipe always re-prompts.
...
- **Config file 0600; host log 0600; log redacts `env` values.** The per-project worker snapshot (`~/.speedwave/host-exec/<project>/config.json`) and the audit log (`~/.speedwave/host-exec/<project>/log`) are both `0600` (the Tauri side pre-creates the log before launching the worker, and the worker's `appendFile` also asserts `0600`). The snapshot may hold a recipe's `env` literals (possibly secrets — the UI warns against that). The log records the full argv, cwd, exit code, status, duration and the confirm decision; it **redacts recipe `env` values** (keys only) but logs the **argv verbatim** — so a recipe whose `args` substitute a `{param}` logs whatever value Claude supplied, which may be sensitive (e.g. a credential in a `psql -c '{sql}'` query).
...
- **A `docker compose up` recipe is host root.** It runs `docker-compose.yml`, which Claude can rewrite (`/workspace:rw`) to add `privileged: true` + `volumes: ["/:/host:rw"]` — full host takeover from a "build" recipe. Strictly worse than a `gradlew test` recipe (which runs as the user, no privilege escalation). Mitigated by forcing `confirm: "ask"` on any `docker`/`docker-compose`/`podman` lifecycle recipe (so a rewritten compose file always re-prompts), but a user who whitelists it has accepted that a recipe can host-root them.
...
- **Windows: Host Exec is non-functional today** — the fd-3 confirm channel isn't wired there, so every recipe (including `confirm: always`) fails closed with "confirmation unavailable". The Desktop UI shows a banner saying so. (Fail-closed, not a hole.)
