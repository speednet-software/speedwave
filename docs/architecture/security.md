...
| `LITELLM_VOLUMES`             | litellm                   | Mount profile is exactly `/config:ro` + `tokens/<project>/llm:/tokens:ro` + `/usage:rw`, and no `network_mode` ([ADR-072](../adr/ADR-072-embedded-per-project-litellm-proxy.md)) |
