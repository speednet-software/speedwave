...
- Networking — `networks: - vzNAT: true`[^5]; the VM inherits the host routing table (and VPN tunnels). There is no Lima port-forwarding / `guestPortRange` configuration. Container-to-host reach uses the static host IP of Lima's user-mode network, `consts::LIMA_VZ_HOST_IP` (`192.168.5.2`).
