...
- Shared SSRF classifier (`is_private_on_premise`, `PrivatePolicy::{AllowLoopback, BlockLoopback}`, `validate_url`) — `desktop/src-tauri/src/url_validation.rs`. This is host-only Tauri policy; it deliberately does NOT live in `speedwave-runtime`, which is pure container-orchestration Rust with no networking policy.
