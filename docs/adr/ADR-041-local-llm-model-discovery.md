...
- **Allow (with warn):** loopback (LLM only), RFC 1918 private[^1], RFC 6598 CGNAT (Tailscale / carrier NAT)[^2], IPv6 ULA (`fc00::/7`)[^3], public IP / public domain (user-written URL is the user's own threat surface), and cleartext `http://` on private addresses.
...
[^2]: RFC 6598 - IANA-Reserved IPv4 Prefix for Shared Address Space (CGN) - https://www.rfc-editor.org/rfc/rfc6598

[^3]: RFC 4193 - Unique Local IPv6 Unicast Addresses (`fc00::/7`) - https://www.rfc-editor.org/rfc/rfc4193

[^4]: llama.cpp server `/v1/models` response includes a `meta.n_ctx_train` field reporting the model's trained context size - https://github.com/ggml-org/llama.cpp/blob/master/tools/server/README.md

[^5]: LM Studio REST API `GET /api/v1/models` response includes a `max_context_length` field - https://lmstudio.ai/docs/developer/rest/list

[^6]: Docker Desktop networking: `host.docker.internal` resolves to the internal IP address of the host from inside a container - https://docs.docker.com/desktop/networking/

[^7]: Ollama does not implement the Anthropic `/v1/messages/count_tokens` endpoint, tracked at ollama/ollama#13949 - https://github.com/ollama/ollama/issues/13949
