...
**Guards.** `desktop/src-tauri/src/installer_hooks.rs` pins the hook: the set of `RMDir /r` targets in PREINSTALL must equal the directory resources of `tauri.windows.conf.json`, so a new directory resource fails the test until the hook clears it, and every removal must sit after the sweep, inside the `${PRODUCTNAME}` guard. The Windows leg of `.github/workflows/desktop-build.yml` (push builds only) runs `scripts/check-nsis-update-reset.ps1` against the installer it just built: install with `/S`, plant a file under `build-context` that the release does not ship, install again with `/S /UPDATE`, and fail if the planted file survives.
