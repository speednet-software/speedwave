...
4. Backend emits JSON Patches: `Remove` on the assistant entry's index, `Replace` on the user entry setting `edited_at = now`.
5. Backend kills the active `claude` child for the session (there should be none because `!streaming`, but defend against stale processes).
6. Backend spawns `claude --resume <session_id> --resume-session-at <user_uuid>` via `ContainerRuntime::exec`[^3] with the usual `--output-format stream-json`[^4] flags. Output routes into the normal patch pipeline.
...
enum RetryError {
    ...
    ResumeFailed(String), // spawn failed; state-tree unchanged
}
```

Each error corresponds to a guard executed in the order above. `ResumeFailed` means the `Remove` patch was NOT emitted (transactional: if we cannot resume, we do not mutate state). Implementation: construct the spawn, test it, and only emit patches after the child process is confirmed running.
...
- Retry adds one additional `claude` process lifecycle per retry click. The existing process management in `ContainerRuntime::exec`[^3] handles this.
...
- Retry does not restore pre-retry session context if the retry itself fails (e.g. rate limit on the resumed turn). The state-tree has already been mutated by the `Remove` patch. The user sees the user message with `edited_at` and no assistant reply — same as if they had just typed it. Recovery: click Send in the composer to re-issue. Accepted as cheap to recover from.
...
[^3]: Speedwave runtime `ContainerRuntime::exec` — process spawning inside the active claude container: `../../crates/speedwave-runtime/src/`
