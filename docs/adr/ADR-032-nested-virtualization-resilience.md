...
- **VM detection** — on Windows, `check_os_warnings()` runs `Get-CimInstance Win32_ComputerSystem` and checks `Model`/`Manufacturer` against known VM vendor strings, logging a non-blocking warning. `Get-CimInstance` is used over the deprecated `Get-WmiObject`[^5] for PowerShell 5.1 and 7+ compatibility.
...
[^1]: [What is Nested Virtualization for Hyper-V? (Microsoft Learn)](https://learn.microsoft.com/en-us/windows-server/virtualization/hyper-v/nested-virtualization): WSL2 uses a subset of the Hyper-V architecture ("Virtual Machine Platform"); nested virtualization lets a Hyper-V-based guest run its own hypervisor.

[^2]: [Teams/Dpkg/FAQ (Debian Wiki)](https://wiki.debian.org/Teams/Dpkg/FAQ): `--force-unsafe-io` avoids `fsync(2)` calls on unpacked filesystem files (still fsyncs the dpkg database), trading crash-safety for speed.

[^3]: [apt.conf(5) (Ubuntu manpages)](https://manpages.ubuntu.com/manpages/xenial/man5/apt.conf.5.html): `Acquire::Retries` sets the number of times APT retries failed downloads.

[^4]: [`std::thread::scope` (Rust standard library documentation)](https://doc.rust-lang.org/std/thread/fn.scope.html): spawns scoped threads that are guaranteed to be joined before the scope returns.

[^5]: [`Get-WmiObject` (Microsoft.PowerShell.Management), PowerShell (Microsoft Learn)](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.management/get-wmiobject?view=powershell-5.1): "Starting in PowerShell 3.0, this cmdlet has been superseded by `Get-CimInstance`."
