...
The frontend accumulates `MessageBlock[]` in `currentBlocks` during an assistant turn. Each `MessageBlock` is one of: `text`, `thinking`, `tool_use`, `ask_user`, or `error`. Only the `Result` chunk finalizes the turn — it moves `currentBlocks` into the `messages` array as a complete `ChatMessage { role, blocks, timestamp }` and captures `SessionStats` (session_id, total_cost_usd, usage). The `assistant` message type from Claude is intentionally ignored — it duplicates content already streamed via `stream_event`.
