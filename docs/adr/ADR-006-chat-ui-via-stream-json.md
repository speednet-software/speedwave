...
Run Claude Code headless (`claude -p`) with `--output-format=stream-json --input-format=stream-json --include-partial-messages`[^1][^2]. Claude emits typed NDJSON on stdout; the host writes user messages (and control responses) to stdin. The process stays alive across turns, so one subprocess serves a whole conversation.

## Why

- Headless mode (`-p`) plus `stream-json` output yields newline-delimited JSON the host can parse incrementally for real-time token streaming[^1][^2].
- `--input-format=stream-json` makes the channel bidirectional: the same long-lived subprocess handles many turns, preserving session, context, and MCP-hub state[^2].
...
> **Amendment (2026-09-15):** The stdout reader used to reset the parser's block state on every `system` line, which dropped the remaining `input_json_delta` events because Claude Code also emits `system` lines mid-turn: task progress while a subagent or background task runs, hook lifecycle events, and status updates such as compaction[^4]. Block state is now reset by the parser alone, on `message_start` and `message_stop` (the reader never resets it), and a new `ToolInputComplete` chunk carries the full tool input from the complete `assistant` line so the frontend can heal a delta-assembled input that no longer parses. The variant list below is corrected in place.

- Output is typed NDJSON. `--include-partial-messages` makes Claude emit `stream_event` messages wrapping raw Anthropic Messages API events (`message_start`, `content_block_start/delta/stop`, `message_stop`) for real-time token, thinking, and tool-input streaming[^2][^3]. Complete `assistant` lines are not re-rendered (their text already streamed); the parser reads them for the message id and usage (ADR-046) and for the complete tool inputs (`ToolInputComplete`).
...
- `StreamParser` collapses the raw events into the `StreamChunk` tagged enum (`desktop/src-tauri/src/chat.rs`, mirrored 1:1 by the TS `StreamChunk` union in `desktop/src/src/app/models/chat.ts`). It has **13 variants**: `Text`, `Thinking`, `ToolStart`, `ToolInputDelta`, `ToolInputComplete` (the full tool input from the complete `assistant` line; the frontend replaces the delta-assembled input with it, so a tool block survives lost deltas), `ToolResult`, `Result`, `AskUserQuestion`, `Error`, `SystemInit` (model from the system-init message), `RateLimit`, `UserMessageCommit` (commits a UUID onto the latest user entry, ADR-046), and `QueueDrained` (one-slot queued message drained server-side, ADR-045).
...
[^1]: Claude Code headless mode: `-p`/`--print`, `--output-format stream-json`, `--verbose`, `--include-partial-messages`. https://code.claude.com/docs/en/headless

[^2]: Claude Code CLI reference: `--input-format`, `--output-format`, `--include-partial-messages`, `--permission-prompt-tool`, `--resume`, `--dangerously-skip-permissions`, `--mcp-config`, `--strict-mcp-config`. https://code.claude.com/docs/en/cli-reference

[^3]: Claude Agent SDK streaming output: `stream_event` messages wrapping raw Anthropic Messages API events (`message_start`, `content_block_start`, `content_block_delta`, `content_block_stop`, `message_delta`, `message_stop`). https://code.claude.com/docs/en/agent-sdk/streaming-output

[^4]: Claude Agent SDK TypeScript reference: `SDKStatusMessage`, `SDKTaskStartedMessage`, `SDKTaskProgressMessage` ("emitted periodically while a subagent or background task is running"), `SDKTaskNotificationMessage`, `SDKHookStartedMessage`/`SDKHookProgressMessage`/`SDKHookResponseMessage`, all `type: "system"`. https://code.claude.com/docs/en/agent-sdk/typescript
