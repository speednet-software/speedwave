...
> **Amendment (2026-09-15):** The stdout reader used to reset the parser's block state on every `system` line, which dropped the remaining `input_json_delta` events once Claude Code started emitting `system` lines mid-turn (task, hook and status subtypes). Block state is now dropped only at a turn boundary — a terminal chunk or `system/init` (`resets_block_state` in `chat.rs`) — and a new `ToolInputComplete` chunk carries the full tool input from the complete `assistant` line so the frontend can replace a delta-assembled input. The variant list below is corrected in place.

- Output is typed NDJSON. `--include-partial-messages` makes Claude emit `stream_event` messages wrapping raw Anthropic Messages API events (`content_block_start/delta/stop`, `message_stop`) for real-time token, thinking, and tool-input streaming[^2][^3]. Complete `assistant` messages are intentionally ignored — they duplicate content already streamed.
