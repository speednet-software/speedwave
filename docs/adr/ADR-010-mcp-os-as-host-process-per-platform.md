...
- The alias resolves to the per-platform gateway IP — Lima user-mode network static `192.168.5.2` on macOS (`consts::LIMA_VZ_HOST_IP`); on Windows the gateway IP is detected at runtime by parsing `wsl.exe -d <distro> -- sh -c 'ip -4 route show default'` (no hardcoded Windows gateway literal exists).
