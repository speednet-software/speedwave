...
The relay task sniffs SSE frames as they pass and, on stream end, appends exactly one compact JSON line to `/usage/usage.jsonl` (per-project bind mount `usage/<project>/proxy/`). The line uses the exact field names the host aggregator reads (`ts, capture, status, model, response_id, cost_usd, latency_ms, ttft_ms, prompt_tokens, completion_tokens, cache_read, cache_write`); the timestamp is RFC3339-millis with a local colon offset (matching `log_ts::log_timestamp`). `ttft_ms` is the elapsed ms to the first output `text_delta` SSE frame, omitted when no token streamed. Anthropic `input_tokens→prompt_tokens`, `output_tokens→completion_tokens`, `cache_read_input_tokens→cache_read`, `cache_creation_input_tokens→cache_write`; `response_id` from `message_start.message.id`.
