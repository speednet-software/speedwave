...
**No-provider state:** `LlmConfig::is_unconfigured()` (`active_provider().is_none()`) is the single SSOT discriminator for "no LLM provider is usable" — true for a never-touched project, a dangling `active` (points at a missing entry), an unmigrated legacy-v1 config, and an explicit logout alike. This is a first-class state, not a silent fall-through to the Anthropic default: the render guard in `apply_llm_config_in` bails with a choose-a-provider error before the proxy/legacy branching, and Desktop surfaces a `no_provider` status (chat shows "choose a provider", auth gate skips the OAuth wall per R7). `is_logged_out()` is a narrower, `pub(crate)` helper used only to pick bail wording ("Run `speedwave login`" vs "choose a provider") — it is not itself a gate. A fresh project's `migrate_llm` no longer fabricates an Anthropic default; it leaves the config untouched so render refuses to start rather than silently using an uncredentialed Anthropic OAuth session. `speedwave login` calls `set_active_to_anthropic`, creating the override if absent, so both a logged-out and a never-configured project escape the guard on success.
...
The relay task sniffs SSE frames as they pass and, on stream end, appends exactly one compact JSON line to `/usage/usage.jsonl` (per-project bind mount `usage/<project>/proxy/`). The line uses the exact field names the host aggregator reads (`ts, status, model, response_id, provider_kind, provider_id, gen_id, cost_usd, latency_ms, ttft_ms, prompt_tokens, completion_tokens, cache_read, cache_write`; `gen_id`, `cost_usd` and `ttft_ms` are omitted when unset); the timestamp is RFC3339-millis with a local colon offset (matching `log_ts::log_timestamp`). `ttft_ms` is the elapsed ms to the first output `text_delta` SSE frame, omitted when no token streamed. Anthropic `input_tokens→prompt_tokens`, `output_tokens→completion_tokens`, `cache_read_input_tokens→cache_read`, `cache_creation_input_tokens→cache_write`; `response_id` from `message_start.message.id`.
...
**Throughput (tok/s) is decode-only.** The dashboard rate divides output tokens by the decode window `latency_ms − ttft_ms`, not full wall-clock — the industry definition of generation throughput excludes TTFT/prefill[^9]. The aggregator sums `decode_latency_ms_sum` over success records carrying both `latency_ms` and `ttft_ms` (`latency > ttft`); records without `ttft_ms` — including non-streaming/coalesced responses — are excluded from throughput, so the sample is decode-streamed requests only.
...
## Amendment (2026-09-13): Anthropic branches inject no model env at all

SPEED-541 removed the `ANTHROPIC_DEFAULT_MODEL` injection from both Anthropic branches of `compose/llm.rs`: since ADR-087 decision 7 the config self-heal clears any stored model for Anthropic entries before the renderer runs, so the branch was dead code. Claude Code resolves the account default, and the persistent `/model` pick lives in the container's `settings.json` `model` key written by the Desktop (ADR-087 decision 3 amendment, SPEED-539). Consequences for the 2026-09-02 amendment above: the "Both Anthropic branches inject the Settings model as `ANTHROPIC_DEFAULT_MODEL`" decision no longer holds, and `config::is_foreign_anthropic_model` stays load-bearing only in `config.rs` (quarantine at load), not in the renderer. The routed kinds are unchanged. Guard: `assert_model_not_forced` in `compose/mod.rs` now rejects `ANTHROPIC_DEFAULT_MODEL=` as well.
