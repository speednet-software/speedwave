...
SPEED-541 removed the `ANTHROPIC_DEFAULT_MODEL` injection from both Anthropic branches of `compose/llm.rs`: since ADR-088 decision 7 the config self-heal clears any stored model for Anthropic entries before the renderer runs, so the branch was dead code. Claude Code resolves the account default, and the persistent `/model` pick lives in the container's `settings.json` `model` key written by the Desktop (ADR-088 decision 3 amendment, SPEED-539). Consequences for the 2026-09-02 amendment above: the "Both Anthropic branches inject the Settings model as `ANTHROPIC_DEFAULT_MODEL`" decision no longer holds, and `config::is_foreign_anthropic_model` stays load-bearing only in `config.rs` (quarantine at load), not in the renderer. The routed kinds are unchanged. Guard: `assert_model_not_forced` in `compose/mod.rs` now rejects `ANTHROPIC_DEFAULT_MODEL=` as well.
