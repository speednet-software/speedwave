...
7. **Pruning is three-layered.** (a) `prune_old_bundle_images` force-removes every catalogue tag for the old bundle id on each bundle change (`--force` because stopped containers from the previous session block plain `rmi`). (b) `prune_orphan_current_bundle_images` runs at the end of each reconcile and force-removes tags of the current bundle id that are no longer in `enabled_images(active)`. (c) The same prune also runs at the end of every successful `restart_integration_containers`, scoped to the union across **all** projects — so a `disable` toggle in any project actually frees the worker image's disk as soon as no project still uses it (deviates from the original SPW-203 wording "leave the image of the current bundle"; we picked disk recovery over saving a future build).
