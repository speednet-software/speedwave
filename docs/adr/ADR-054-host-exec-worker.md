...
> **Context:** Claude runs in a token-free container whose only language runtimes are `node` + `npm` and `python3` + `pip` (`Containerfile.claude`), plus `git` and a shell — enough to run `.js`/`.py` scripts and install libraries into `/workspace`/`$HOME`. It cannot reach the user's real **host** toolchain (JDK, Gradle, Go, .NET, the host's Docker stack) to build/test/run the project — the most-requested gap, and one users were already filling unsafely with hand-rolled agents on `0.0.0.0` with tokens committed to the repo. (Both interpreters live inside the same hardened, token-free, no-host-reach container; `host_exec` was about executing repo code **on the host** with the user's privileges, which is what this ADR rejects.)
