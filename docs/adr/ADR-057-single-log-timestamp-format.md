...
- **Frontend** — `parseLogLine` pulls the leading timestamp into the time column (nerdctl's unbracketed RFC3339, or a Rust drain prefix, or a bare `[<ISO>]`), then strips a `STDOUT: `/`STDERR: ` drain marker (semantic `SESSION:`/`TOOL:`/`RATE_LIMIT:`/`SYSTEM:` prefixes are kept) **and the worker's own `[<ISO>]` `ts()` stamp** — a compose-container line carries nerdctl's stamp *and* `ts()`'s, so the inline one is the same instant as the column and just noise (if the head had no timestamp, the inline one is promoted instead). `formatTime` **parses any ISO stamp and re-renders it in the host's local zone** (`YYYY-MM-DD HH:MM:SS`) — so a UTC `Z` stamp and a `+02:00` stamp for the same instant land in the column identically; the raw value stays in the `[title]` tooltip. **`sortLogLinesByTime` then interleaves the per-source blocks the backend concatenates into one chronological stream** (parsing each line's ISO instant; a line with no parseable timestamp inherits the previous line's instant so continuations stay attached; the sort is stable). The backend's `merge_log_sources` no longer needs to sort — it just concatenates `compose | desktop | mcp-os | host-exec | claude` in a fixed order.

Both SSOTs render **local time with an explicit offset** (host TZ, propagated into containers via the `TZ` env). A line that reaches the view with two stamps (a drain/nerdctl stamp plus the worker's own `[<ISO>]`) shows just one: the parser keeps the leading one for the column and removes the inline `[<ISO>]` from the message.
...
- The on-disk/raw log line for a drained or compose-container source still carries two stamps (the drain/nerdctl prefix and the worker's `[<ISO>]`); only the `/logs` *view* dedupes — the export ZIP and a `tail -f` of the file see both. Acceptable: they agree, and on-disk redundancy aids forensic correlation.
