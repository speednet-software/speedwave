...
- **Frontend parser** — `BRACKETED_TIME_RE` is extended to accept an ISO timestamp inside the brackets (workers emit `[<ISO>]` via `ts()`); after timestamp extraction a leading `STDOUT: `/`STDERR: ` drain marker is stripped from the message (semantic claude-session prefixes — `SESSION:`/`TOOL:`/`RATE_LIMIT:`/`SYSTEM:` — are kept). `formatTime` **parses any ISO stamp and re-renders it in the host's local zone** (`YYYY-MM-DD HH:MM:SS`) — so a UTC `Z` stamp (nerdctl `--timestamps`) and a `+02:00` stamp for the same instant land in the column identically; the raw value stays in the `[title]` tooltip. (A compose-container line therefore carries two stamps — nerdctl's containerd-level one in the column, the worker's own `[<ISO>]` in the message — that agree once both are localised; the redundancy is harmless.)
