...
> **Amendment (2026-09-16):** `claude plugin list --json` was itself a defect, unrelated to the marketplace-bootstrap problem above: a short-lived Claude Code CLI command started while a claude.ai sign-in is stored and the access token has expired sends an OAuth refresh during startup and can exit before the reply is saved, and by then the server has rotated the refresh token, so the next Claude Code process (the one starting the actual session) is rejected and the stored sign-in is cleared. The entrypoint's `timeout 30 claude plugin list --json` was exactly such a command (observed exiting at ~0.8s on Claude Code 2.1.267, not documented upstream). The install-set check here and the retired-plugin check (ADR-087) now read Claude Code's own install record instead, `${HOME}/.claude/plugins/installed_plugins.json` (a `plugins` map keyed by `name@marketplace`, each value an array of per-scope install records; observed on Claude Code 2.1.267, not documented upstream), so the entrypoint no longer starts a Claude Code process merely to answer "is this plugin installed". `claude plugin install`, `claude plugin uninstall` and `claude plugin marketplace add` still wait for their reply and are unaffected by this amendment.
