...
>
> **Update (ADR-080):** the "binds the WSL vEthernet adapter IP on Windows" note below holds only under **NAT** networking. Under **mirrored** networking (the default) the bridge binds `127.0.0.1` and containers reach it via a guest-side `socat` relay — invisible to the LAN either way.
