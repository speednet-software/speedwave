...
- Concurrent `queue` / `take` / `cancel` arrive from UI input, stream-end handlers, and shutdown paths at once; a `DashMap` gives per-key lock serialization so every outcome is well-formed (queue-first → runs now; take-first → waits one turn).[^1] No global mutex.
...
- **Send immediately on a second concurrent process.** A second process writing the same session JSONL causes messages from both to interleave into one transcript rather than staying isolated.[^2]
...
[^1]: DashMap divides its data into independently-locked shards keyed by hash, so operations on different keys do not contend for a single global lock: https://docs.rs/dashmap/latest/dashmap/struct.DashMap.html

[^2]: "If you resume the same session in two terminals without forking, messages from both interleave into one transcript." Claude Code docs, Manage sessions: https://code.claude.com/docs/en/sessions
