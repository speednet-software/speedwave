...
2. Bind a one-shot callback server on **`127.0.0.1:<port>`** — the browser-side loopback, deliberately distinct from `host_bind_address()` (which targets container reach — the WSL adapter IP under **NAT**, but loopback fronted by the ADR-080 guest relay under **mirrored** mode). A fixed `redirect_port` is supported for IdPs that require a registered redirect URI; otherwise an ephemeral port is used.[^5]
