...
1. **`metadata` automount** — `setup_wizard::ensure_wsl_distro_metadata(TerminateOnChange)` writes `[automount]\noptions = "metadata,uid=1000,gid=1000,umask=022"` (uid/gid derived from the `consts::CONTAINER_USER_UNPRIVILEGED` SSOT via `container_uid_gid()`). `metadata` makes drvfs honor Linux mode bits so `chmod 0600` works. The `uid=`/`gid=` part is **best-effort, not load-bearing**: the imported distro has no `[user]` default → WSL prepends the default-user uid (root → 0) ahead of our option, and the prepended uid wins, so the mount stays uid 0. The `TerminateOnChange` argument gates the post-write `wsl --terminate`: `Yes` right after `wsl --import` (no container yet — safe), `IfIdle` on the startup migration for existing distros (terminate only when no container is running, so the metadata mount applies before the first container start; defer when containers are live to avoid killing them mid-start). **Write verification:** `ensure_wsl_distro_metadata` re-reads `wsl.conf` after the mutation and returns `Err` when the uid is still absent — a silently-failed write (e.g. a read-only `wsl.conf`) no longer reports success, so the caller surfaces the failure instead of proceeding to a uid-0 mount that breaks `/login`. **Migration boot:** with `IfIdle`, a pre-existing distro that is idle at startup is terminated and the metadata mount applies immediately, so the first `/login` after the update works; only when containers are already running does the new config wait for the next natural restart. Fresh imports (`Yes`) are unaffected.
