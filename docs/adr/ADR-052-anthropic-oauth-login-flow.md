...
Fix: the Speedwave distro's internal `/etc/wsl.conf` must declare `[automount]\noptions = "metadata,uid=1000,gid=1000,umask=022"`. `metadata` makes drvfs honor Linux permission bits (so `chmod 0600` works); `uid=1000,gid=1000` makes the mount **owned by the container user**. The second half is non-negotiable: the imported distro has no default user, so the mount defaults to **uid 0**, and once `metadata` enforces ownership the uid-1000 container can no longer write `/home/speedwave` — its entrypoint hits `EACCES` on the first `mkdir`/`ln` and exits under `set -euo pipefail`, leaving the container `Exited` ("cannot exec in a stopped state"). Bare `options = "metadata"` (an earlier iteration of this fix) reintroduced exactly that failure. `setup_wizard::ensure_wsl_distro_metadata(TerminateOnChange)` writes this, and self-upgrades a bare `metadata` line left by that earlier build. The `TerminateOnChange` argument gates the post-write `wsl --terminate` (which applies the new `wsl.conf` immediately but kills every process in the distro): `Yes` right after `wsl --import` (no container exists yet, so terminating is safe), `No` on the startup migration for distros created before this fix (containers may already be running — a terminate there kills them mid-start, surfacing as `cannot exec in a stopped state`; instead the new `wsl.conf` applies on the distro's next natural restart). This is distro-internal config — distinct from the host `%USERPROFILE%\.wslconfig` managed by `ensure_wslconfig_vpn_compat` (ADR-067). macOS (Lima VirtioFS) is unaffected; it honors chmod natively.
