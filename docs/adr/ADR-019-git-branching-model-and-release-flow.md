...
- **CLI:** `speedwave self-update` downloads the latest GitHub release and replaces the binary in place — `crates/speedwave-cli/src/main.rs` (`run_self_update`, via the `self_update` crate[^5]). On macOS the CLI is bundled inside `Speedwave.app` (`Contents/Resources/cli/speedwave`) and copied to `~/.local/bin/<name>` by `desktop/src-tauri/src/setup_wizard.rs` (`link_cli` → `copy_cli_binary`), where `<name>` carries the instance (`speedwave` for the production data dir, `speedwave-<suffix>` otherwise — ADR-016), so updating the Desktop app re-copies the fresh CLI; a binary that detects it is itself running from inside an `.app` bundle (`is_app_bundle`) refuses self-update and points the user at the Desktop app instead.
