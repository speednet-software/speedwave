...
Drive the real, installed desktop binary on a real machine via SSH orchestration, using WebdriverIO over the W3C WebDriver protocol. Both supported platforms (macOS and Windows) embed the same community crate `tauri-plugin-webdriver`, which runs a W3C WebDriver server on `127.0.0.1:4445` inside the app — no external driver process. The crate is compiled only when the `e2e` Cargo feature is set, so production releases never include it.
...
- An embedded in-app WebDriver server works identically on macOS (WKWebView) and Windows (WebView2) with zero external dependencies — Apple's WKWebView does not expose a WebDriver interface, so the official external `tauri-driver` could not be used on macOS, and one mechanism for both platforms is simpler than two.
...
- WebDriver commands operate in the webview context only — they cannot reach Tauri backend state, tokens, or the host filesystem.
...
- Tauri v2 WebDriver Testing guide — https://v2.tauri.app/develop/tests/webdriver/
- tauri-plugin-webdriver crate — https://lib.rs/crates/tauri-plugin-webdriver
