# ADR-088: Neural PII Detection with the Redact Model Ported to burn
...
- The Desktop build grows by burn, cubecl and wgpu, and by about 25 MB of model artifact; the proxy image is unchanged (it only calls the host, ADR-089). Root `make check-clippy` compiles `speedwave-pii-ner`.
