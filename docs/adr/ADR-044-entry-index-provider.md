...
- The state-tree's `entries: Vec<ConversationEntry>` is sparse after removes (holes where entries were removed). The Vec is not literally sparse in memory — it's a `Vec<ConversationEntry>` with some `ConversationEntry.removed = true` or an `Option`, depending on the final schema choice — and the `index` field on each entry is the authoritative addressing key, not the Vec position. Rendering iterates and filters `removed` entries; Tracking uses `entry.index`.
...
- The counter-plus-sparse-Vec shape diverges from the "natural" `entries: Vec<Entry>` indexed by position. A new contributor must know that `entries[k]` is not the entry with index `k`. Mitigations: a helper `find_by_index(entries, idx)`; the patch-helper layer (ADR-042) hides the lookup from call sites.
- `start_from` is O(history size) at resume time. With the 100 MB MsgStore cap (ADR-043), worst case is ~100 000 patches — a millisecond-scale scan on modern hardware, not worth optimizing.
