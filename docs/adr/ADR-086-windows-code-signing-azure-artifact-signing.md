...
- The `release` environment has a deployment-branch policy: only runs whose ref is the protected `main` branch may enter it; tags are excluded because tag creation is not protected in this repository. Since the OIDC subject carries the environment name only when the job references that environment, and a job on any other ref is rejected before it starts, a signature can only come from code that reached `main` through its pull-request gate, and a fork pull request cannot obtain a write-scoped token at all.[^5][^14] Adding required reviewers on top would gate every release build, not only signing.
