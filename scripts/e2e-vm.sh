...
linux_rsync_to() {
    ...
    rsync -az -e "ssh $LINUX_SSH_OPTS" --delete \
        ...
        --exclude .e2e-artifacts --exclude .git --exclude .build \
        --exclude build-context \
        --exclude nerdctl-full --exclude lima --exclude nodejs \
        "$src" "${LINUX_HOST}:${dst}"
}
...
windows_rsync_to() {
    ...
    local -a tar_excludes=(--exclude=node_modules --exclude=target --exclude=dist --exclude=.e2e-artifacts --exclude=.git '--exclude=._*' --exclude=.angular --exclude=.build --exclude=build-context --exclude=nerdctl-full --exclude=lima --exclude=nodejs)
