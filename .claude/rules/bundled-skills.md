...
- Working files a skill creates in the user's project live under `/workspace/.speedwave/`: `.speedwave/scratch/<feature-slug>/` for the local markdown tracker (Matt's inner layout `spec.md`, `issues/NN-slug.md`, `map.md` kept), `.speedwave/handoffs/<date>-<slug>.md` for handoff documents and `.speedwave/reports/` for the architecture report. Team configuration written by `/speedwave-setup` (`docs/agents/*.md`, `SPEEDWAVE.md`, the `## Agent skills` block in `CLAUDE.md` or `AGENTS.md`) is a team document, not a working file.
...
3. Working-file paths: `.scratch/` to `.speedwave/scratch/`; `speedwave-handoff` saves to `/workspace/.speedwave/handoffs/` and `speedwave-improve-codebase-architecture` to `/workspace/.speedwave/reports/` instead of the OS temp dir (no browser opener exists in the container).
...
`superpowers` left `defaults.rs::BUNDLED_PLUGINS` with ADR-087 because its skills and SessionStart hook competed with the vendored set. `containers/entrypoint.sh` uninstalls it once, only when the v2 bundled-plugins marker records that Speedwave installed it (a plugin Claude Code already reported installed is recorded as `<id>#found` and never retired), then drops the marker line; a failed removal keeps the line and the bundled-install pass carries it into its rebuilt marker. The uninstall is bounded by `timeout` and non-fatal. Because the uninstall step has to name the plugin, the repo-wide grep for that name legitimately hits the entrypoint, its bats tests, the `defaults.rs` doc comment and test, `.gitignore` and the git-workflow rule (the host-side plugin still writes `docs/superpowers/`), this rule, the ADR index and ADR-077/087, nothing else.
