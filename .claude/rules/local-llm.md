...
- **Anthropic kinds inject no model env at all** (SPEED-541: the prior `ANTHROPIC_DEFAULT_MODEL` injection was dead code — the project config never carries an Anthropic model past the config self-heal, `config.rs::clear_active_anthropic_model`/`clear_anthropic_models`). Claude Code resolves the account default; the persistent `/model` pick lives in the in-container `settings.json` `model` key, written by the Desktop (SPEED-539, ADR-088 amendment). Test-guarded: `assert_model_not_forced` in `compose/mod.rs`, extended to also reject `ANTHROPIC_DEFAULT_MODEL=`.
- **Speedwave never sets `CLAUDE_CODE_EFFORT_LEVEL`.** Per `env-vars.md`, it "Takes precedence over `--effort`, `/effort`, and the `modelSettings` and `effortLevel` settings" — setting it in compose would outrank the user's live `/effort` and the `--effort` launch flag, and compose bakes env in at container create (no way to change it without recreating the container). Test-guarded: `test_claude_env_has_no_effort_level` plus `assert_no_effort_level_forced` in `compose/mod.rs`, covering the Anthropic proxy path, the Anthropic direct path, and routed local/OpenRouter.
...
- **Anthropic kinds pin `ANTHROPIC_DEFAULT_{SONNET,HAIKU}_MODEL` only**, each alias to its latest catalog model, with the `[1m]` suffix only where the model supports a 1M context: SONNET because Sonnet 5 needs no usage credits for 1M on any plan (without the pin the `sonnet` alias drops to the 200k bare id), HAIKU because it keeps the undated catalog id. OPUS is deliberately omitted: 1M for Opus needs usage credits on Pro, so a pinned `claude-opus-5[1m]` would force everything resolving the `opus` alias (subagents with `model: opus`, `opusplan`) onto a paid window (test `anthropic_default_models_env_omits_the_plan_dependent_opus_alias`); the composer picks the Opus window per plan. FABLE is deliberately omitted too (it resolves natively; test `anthropic_default_models_env_omits_fable_alias`). `[1m]` is real Claude Code model-id syntax — it must stay quoted in compose YAML (Go/nerdctl chokes on `[`); never strip it.
