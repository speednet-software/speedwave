...
- **Anthropic kinds inject no model env at all** (SPEED-541: the prior `ANTHROPIC_DEFAULT_MODEL` injection was dead code — the project config never carries an Anthropic model past the config self-heal, `config.rs::clear_active_anthropic_model`/`clear_anthropic_models`). Claude Code resolves the account default; the persistent `/model` pick lives in the in-container `settings.json` `model` key, written by the Desktop (SPEED-539, ADR-087 amendment). Test-guarded: `assert_model_not_forced` in `compose/mod.rs`, extended to also reject `ANTHROPIC_DEFAULT_MODEL=`.
