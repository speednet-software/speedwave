...
- Any new host-side WebSocket relay must be a `HostBridge` (`desktop/src-tauri/src/bridges/host_bridge.rs`) — never a hand-rolled listener/lock-file/token stack; the skeleton owns the audited security model (0o600 lock, constant-time token compare, Origin policy, watchdog). The one HTTP host listener, the PII NER detector (`pii_ner_service.rs`, ADR-090), reuses the skeleton's bind, token and compare helpers plus the host worker lock schema; a new HTTP listener follows it or becomes a `HostBridge`.
- **PII NER stays on this machine and its consumer is pinned.** The proxy sends request text only to `proxy.json` `ner.url`, which `SecurityRule::SpeedwaveProxyNerUrl` and the proxy's own load check restrict to `http://host.docker.internal:<port>`; the AES-SIV key never leaves the proxy container (the host returns offsets, the proxy seals). Never point `ner.url` anywhere else, never add a cloud detector path, and never send the detector anything but the leaves `pii::collect_scan_leaves` already scans.
