...
- **Bumping the nerdctl pin, the Lima guest image or the WSL rootfs requires a green `make test-engine-contract`** — that suite pins the real-engine semantics the self-heal (#932) and the compose up deadline rely on: nerdctl's name-store/classifier/flock phrases, containerd's task-bundle collision text, and the guest coreutils `timeout --verbose` line. A bump that changes those phrases or exit codes must be caught here before it reaches the renderer.
