...
- **Bundle scripts copy `containers/` and `crates/pii-engine` through `copy_tree` (.sh) / `Copy-Tree` (.ps1)**, which skip `bundle.rs::HOST_BUILD_OUTPUT_DIRS` up front. Never a whole-tree `cp -r`/`Copy-Item -Recurse` there: `make test` runs `test-proxy` in parallel, and a `cargo build` writing `containers/proxy/target` races the copy (a vanished `rustcXXXX` temp file failed the bundle).
