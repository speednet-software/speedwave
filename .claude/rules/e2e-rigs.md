...
- The preflight (`desktop/e2e/helpers/preflight.ts`) sends one real request to `LOCAL_LLM_MODEL` (`POST /v1/messages`, `max_tokens` 16, HTTP 200 with a `stop_reason`, a 429 retried): a server that lists models but does not answer for that one skips spec 11 and the local-provider test of spec 20 while spec 12 still runs, and an unreachable server skips all three. A model listed in `/v1/models` is no proof it is served (LiteLLM lists a model llama-swap no longer routes).
