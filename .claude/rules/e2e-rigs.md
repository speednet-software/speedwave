...
- macOS only: every app launch finds the VM settled — `run_macos_e2e` stops an existing instance through the bundled `limactl stop` (prod `LIMA_HOME`, output kept in the pipeline log) before launching, and only then SIGKILLs stray `limactl` processes. A bare `pkill -f limactl` (SIGTERM) starts a graceful shutdown during which `limactl list` still says Running for tens of seconds while SSH resets (`kex_exchange_identification`): the reset-only launch after the bats suites then saw every engine call fail and no `project-pill`. Guard: `_tests/e2e/e2e-vm-excludes.bats`.
