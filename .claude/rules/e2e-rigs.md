...
- macOS only: `run_macos_e2e` never stops or kills the VM before launching the app — the reset-only launch adopts the VM the bats step started (`ensure_ready` sees it Running). A `pkill -f limactl` there SIGTERMed the hostagent into a graceful shutdown during which `limactl list` still said Running for tens of seconds while SSH reset (`kex_exchange_identification`), so every engine call of the fresh app failed and spec 07 found no `project-pill`. Guard: `_tests/e2e/e2e-vm-excludes.bats`.
...
- Once `GET /v1/models` answers, the preflight (`desktop/e2e/helpers/preflight.ts`) sends one real request to `LOCAL_LLM_MODEL` (`POST /v1/messages`, `max_tokens` 16, HTTP 200 with a `stop_reason`, a 429 retried): a model that does not answer skips only spec 11, the one spec that talks to it (`localModelUnavailable()`); specs 12 and 20 need only the server (`localLlmUnreachable()`), and an unreachable server skips spec 11, the live-server tests of spec 12 and the local-provider test of spec 20. A model listed in `/v1/models` is no proof it is served (LiteLLM lists a model llama-swap no longer routes).
...
- No object an e2e script hands back to WebDriver (a `done(...)` payload, a `browser.execute` result) may carry an `error`, `stacktrace` or `stackTrace` key: WebdriverIO reads such a value as a W3C WebDriver error response (`webdriver`'s `isSuccessfulResponse`) and throws `WebDriverError` instead of returning it, so `tauri-invoke.ts` returns a rejected invoke as `{ ok: false, reason }`. Guard: `_tests/desktop/e2e-invoke-helper.bats`, in `make test-desktop-config`.
- An element a retry loop clicks is looked up inside each attempt (`projects.ts::openProjectSwitcher`), never through a handle taken before the loop: tauri-plugin-webdriver reports a click on a detached element as a JavaScript error, which WebdriverIO does not answer with a fresh lookup, so one re-render (the chat view draws the project pill in a separate header per project status) leaves every retry clicking the dead handle.
