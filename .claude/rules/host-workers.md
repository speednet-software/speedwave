...
- **WSL2 mirrored-relay lifecycle is mandatory (ADR-080):** every host listener a container must reach needs an ensure after bind/spawn, a remove on stop, and a periodic (~30 s) watchdog re-ensure — the relay is a transient unit inside the distro; a WSL restart wipes it while the host process survives. Compose-side URLs translate via `compose::container_facing_port` automatically; a listener missing any of the three steps is unreachable from containers under mirrored mode (the Windows default). The lifecycle rides the owning wrapper, never call-site wiring: `HostBridge` does all three internally, and host MCP workers (mcp-os, oauth) are supervised as `mirror_relay::RelayedWorker`, whose `new`/`is_alive`/`respawn`/`stop` carry ensure/re-ensure/swap/teardown (a respawn stop defers teardown via `RetiredRelay` so a port-reusing replacement adopts the relay). A new host worker wraps its `HostMcpProcess` in `RelayedWorker` (or is a `HostBridge`) instead of calling `mirror_relay` functions at spawn/stop sites.
- **The PII NER detector is an in-process HTTP service, not a spawned worker** (`desktop/src-tauri/src/pii_ner_service.rs`, ADR-091): it binds through `host_bridge::bind_with_retry`, persists its token with `load_or_create_persistent_token`, writes `<data_dir>/pii-ner.lock.json` in the `host_mcp_process::lock` schema (`LockService::PiiNer`, PID = the app), runs the relay ensure/remove/re-ensure itself, and is stopped by `run_exit_cleanup`. The compose renderer reads that lock (`compose::pii_ner::live_service_in`, PID-gated) to render the proxy's `ner` section; `reconcile_compose_port` regenerates when either the mcp-os or the detector URL is stale. The CLI never starts it.
