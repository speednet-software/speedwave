...
paths:
  ...
  - 'desktop/src-tauri/src/plugin_host_bridge.rs'
...
- **Compose injection:** `compose/plugins.rs::apply_plugins()` + `plugin.rs::generate_plugin_service()`. Honored knobs: `Containerfile` (required for MCP plugins), `extra_env`, `mem_limit`, `cpu_limit`, `token_mount` (`read_write` rejected for plugins — `:ro` only, no exceptions). The manifest `port` field is deprecated and ignored: all workers listen on `consts::PORT_WORKER` (3000); a declared port only logs a warning.
- **Hub discovery:** `WORKER_<SLUG_UPPER>_URL` (hyphens → underscores). Rust SSOT `plugin::derive_worker_env`; TS mirror `mcp-servers/hub/src/worker-env.ts::deriveWorkerEnv` (fixture-mirrored tests — update both). No parallel discovery mechanism.
...
- Claude-side resources: plugin ships `claude-resources/`; `entrypoint.sh` iterates `SPEEDWAVE_PLUGINS` (comma-separated enabled slugs) and symlinks them.
