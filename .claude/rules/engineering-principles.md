...
- **No lint suppression** — no `#[allow(...)]` anywhere; fix the underlying issue. Two exceptions only: `#[allow(clippy::unwrap_used, clippy::expect_used)]` on `#[cfg(test)] mod tests`, and `#[allow(unsafe_code)]` on a narrowly-scoped OS-FFI boundary fn/module where every `unsafe` block carries a SAFETY comment (the `job_object.rs` / `fs_perms.rs` / `managed_config.rs` / `mic_permission_cmd.rs` pattern). Dead code is removed, not silenced: test-only items go behind `#[cfg(test)]`; serde-required-but-unread fields get a `_` prefix + `#[serde(rename = "...")]`.
