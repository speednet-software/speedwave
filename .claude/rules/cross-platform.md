...
- BOM polarity is asymmetric: `.ps1` files must be UTF-8 **with** BOM (PowerShell falls back to the system locale reading a BOM-less `.ps1`; editing one requires `make generate-installer-nsh`); `.vbs` and `.sh` must be BOM-**free** (wscript and shebangs choke on it). The NSIS installer's `$PLUGINSDIR` copies of `sweep.ps1`/`firewall.ps1` are BOM-less on purpose: the Unicode installer's `FileWrite` writes ACP bytes, which PowerShell's BOM-less fallback reads back in the same code page, so `generate-installer-nsh.sh` strips the BOM. Note: `sweep.ps1`, `firewall.ps1`, `setup-dev-windows.ps1`, `install-vulkan-sdk.ps1`, `sign-windows-binaries.ps1`, and `bundle-build-context.ps1` carry the BOM in-repo; `sweep.ps1` and `firewall.ps1` have theirs test-pinned (`installer_hooks.rs::ps1_sources_have_utf8_bom`, next to `run-hidden.vbs`'s BOM-freeness), as do `sign-windows-binaries.ps1` and `setup-dev-windows.ps1` (`_tests/desktop/sign-windows-binaries.bats`, `_tests/desktop/setup-dev-windows.bats`); `install-vulkan-sdk.ps1` and `bundle-build-context.ps1` are unpinned.
