...
- BOM polarity is asymmetric: `.ps1` files must be UTF-8 **with** BOM (PowerShell falls back to the system locale reading a BOM-less `.ps1`; editing one requires `make generate-installer-nsh`); `.vbs` and `.sh` must be BOM-**free** (wscript and shebangs choke on it). The NSIS installer's `$PLUGINSDIR` copies of `sweep.ps1`/`firewall.ps1` are BOM-less on purpose: the Unicode installer's `FileWrite` converts to the ANSI code page, the same one PowerShell's BOM-less fallback uses, so `generate-installer-nsh.sh` strips the BOM. That conversion is lossy (characters outside the installing machine's code page become best-fit or `?`), so keep non-ASCII out of string literals in those two scripts. Guards: `_tests/desktop/ps1-utf8-bom.bats` pins the BOM on every tracked `.ps1`; `installer_hooks.rs` rejects a doubled BOM in `sweep.ps1`/`firewall.ps1`, any BOM in the generated `installer-hooks.nsh`, and a BOM in `run-hidden.vbs`.
