...
- Addressing is a pluggable strategy (`compose/addressing.rs`, ADR-067): the `HostAddressingComputer` trait (`LimaStatic`/`WslDetector`/`Unsupported`) behind two `RwLock`s — a value cache and a strategy slot; the resolved `HostAddressing` carries an explicit `AddressingMode` (never inferred from the gateway IP). `invalidate_host_addressing_cache` bumps a generation counter **before** clearing the value cache, and `host_addressing` stores its result only when the generation is unchanged — otherwise a `compute()` still in flight (on Windows a multi-second `wsl.exe` probe) lands its pre-`wsl --shutdown` address after the flush and the next reader gets the stale IP; keep the bump before the clear. In tests pin only via the RAII helpers `pin_direct_addressing`/`pin_mirrored_addressing`/`pin_addressing_computer` under `#[serial_test::serial(host_addressing)]`; never mutate a raw cached IP. The slot is process-global, so every test that reads addressing — directly or through `HostBridge::new`/`start`, a compose render, or a host-worker spawn/probe — carries `#[serial_test::parallel(host_addressing)]`; unkeyed, it can observe another test's pin or cache a stale value over it. A reader that also holds `#[serial(env)]` lists the env key last (the later attribute is the outer lock) — reversing the order on one test deadlocks the suite. Under `cfg(test)`/the `test-support` feature the default computer is a deterministic fixture — desktop tests never spawn `wsl.exe` through addressing.
...
- Git for Windows ships no `rsync`: a script that CI runs under `shell: bash` on `windows-latest` (`bundle-build-context.sh`) copies a filtered tree with `find -print0 | tar --null -T -` (GNU tar in Git Bash, bsdtar on macOS both accept it), never `rsync`. The extracting side drains the pipe (`{ tar -x… && cat >/dev/null; }`): `tar -x` exits at the end-of-archive blocks, and a writer still flushing record padding then hits EPIPE, which the Actions runner (SIGPIPE ignored) surfaces as bsdtar `Write error`.
...
- BOM polarity is asymmetric: `.ps1` files must be UTF-8 **with** BOM (PowerShell falls back to the system locale reading a BOM-less `.ps1`; editing one requires `make generate-installer-nsh`); `.sh` files must be BOM-**free** (a shebang chokes on it). The NSIS installer's `$PLUGINSDIR` copies of `sweep.ps1`/`firewall.ps1`/`reset.ps1` are BOM-less on purpose: the Unicode installer's `FileWrite` converts to the ANSI code page, the same one PowerShell's BOM-less fallback uses, so `generate-installer-nsh.sh` strips the BOM. That conversion is lossy (characters outside the installing machine's code page become best-fit or `?`), so keep non-ASCII out of string literals in those three scripts. Guards: `_tests/desktop/ps1-utf8-bom.bats` pins the BOM on every tracked `.ps1`; `installer_hooks.rs` rejects a doubled BOM in `sweep.ps1`/`firewall.ps1`/`reset.ps1`, and any BOM in the generated `installer-hooks.nsh`.
- Installer PowerShell (`sweep.ps1`, `reset.ps1`, `firewall.ps1`) has two traps that pass every text test. The comma binds tighter than `+`, so `@($a, $b + 'x', $c)` is the four elements `$a`, `$b`, `'x'`, `$c`: compute an element into a variable before listing it (the sweep's lock probe listed the `nodejs\` directory this way and failed every update from v0.12.0, ADR-048 amendment of 2026-09-24). Under `$ErrorActionPreference = 'Stop'`, `Write-Error` ends a `-File` script with exit code 1 before the `exit N` after it: report with `[Console]::Error.WriteLine`, then `exit N`. Build paths with `[System.IO.Path]::Combine`, never `'\'` concatenation, so that `_tests/desktop/installer-{reset,sweep}.bats` run the scripts unchanged under `pwsh` on macOS. The NSIS hooks start them only through `SPEEDWAVE_RUN_HIDDEN` (`CreateProcessW` with `CREATE_NO_WINDOW`): never through `wscript`/VBScript, which strips the quotes around a `$PLUGINSDIR` path that contains a space, and never through `nsExec`, whose console window flashes.
