...
- Git for Windows ships no `rsync`: a script that CI runs under `shell: bash` on `windows-latest` (`bundle-build-context.sh`) copies a filtered tree with `find -print0 | tar --null -T -` (GNU tar in Git Bash, bsdtar on macOS both accept it), never `rsync`.
