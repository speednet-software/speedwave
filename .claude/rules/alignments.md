...
- Windows dev-toolchain Chocolatey package list: `scripts/setup-dev-windows.ps1` (`choco install` line, the SSOT) ↔ the `choco install` block in `docs/contributing/development-setup.md`. The Makefile comment deliberately carries no list — keep it that way.
