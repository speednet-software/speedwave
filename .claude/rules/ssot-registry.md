...
- `mcp-servers/policies/templates/*.yaml`: the PII policy template presets (`strict`/`gdpr-art32`/`eu-ai-act-art5`). Not code: only Rust `pii_policy.rs` reads them, via `include_str!` (the pre-WASM TS `template-loader.ts` reader is gone, ADR-081). Never hand-list a template's category mapping in Rust, edit the YAML instead.
- `crates/pii-engine` token format + `TOKEN_SPAN_RE` (`scan.rs`, `pub const`): the one PII token span shape, `[CATEGORY:TOKEN_<base64url(AES-SIV ciphertext)>]`, and the one regex that recognizes it. Never hand-write the token format at a call site; both the native proxy and the WASM-wrapped hub scan/detokenize through this regex (ADR-081).
