...
- `compose/addressing.rs` — `host_bind_address()`/`host_gateway_ip()`: every host TCP listener bind and every container→host gateway IP. `mirror_relay_port(bind)` (ADR-079) is the deterministic container-facing relay port under WSL2 mirrored mode; the compose URL injectors and container-facing lock naming call its exported wrapper `container_facing_port(bind)` (relay port, or unchanged off-mirrored) — one SSOT shared with the guest `socat` relay supervisor.
