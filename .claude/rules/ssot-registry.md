...
- `compose/addressing.rs` — `host_bind_address()`/`host_gateway_ip()`: every host TCP listener bind and every container→host gateway IP. `mirror_relay_port(bind)` (ADR-079) is the deterministic container-facing relay port under WSL2 mirrored mode — the single SSOT for both the compose URL injector and the guest `socat` relay supervisor.
