...
describe('slack auth enforcement', () => {
  it('exits with code 1 when MCP_SLACK_AUTH_TOKEN is not set', async () => {
    ...
    try {
      await exec('node', ['dist/index.js'], {
        cwd,
        env: { ...process.env, MCP_SLACK_AUTH_TOKEN: '' },
        timeout: 5000,
      });
      ...
      expect(execError.stderr).toContain('MCP_SLACK_AUTH_TOKEN is required');
    }
  });
  ...
  describe('middleware wiring', () => {
    ...
    it('returns 401 for requests without Bearer token', async () => {
      await startServerWithAuth('test-slack-token');
