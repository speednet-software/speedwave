...
describe('gitlab auth enforcement', () => {
  it('exits with code 1 when MCP_GITLAB_AUTH_TOKEN is not set', async () => {
    ...
    try {
      await exec('node', ['dist/index.js'], {
        cwd,
        env: { ...process.env, MCP_GITLAB_AUTH_TOKEN: '' },
        timeout: 5000,
      });
      ...
      expect(execError.stderr).toContain('MCP_GITLAB_AUTH_TOKEN is required');
    }
  });
