...
describe('sharepoint auth enforcement', () => {
  it('exits with code 1 when MCP_SHAREPOINT_AUTH_TOKEN is not set', async () => {
    ...
    try {
      await exec('node', ['dist/index.js'], {
        cwd,
        env: { ...process.env, MCP_SHAREPOINT_AUTH_TOKEN: '' },
        timeout: 5000,
      });
      ...
      expect(execError.stderr).toContain('MCP_SHAREPOINT_AUTH_TOKEN is required');
    }
  });
  ...
  describe('middleware wiring', () => {
    ...
    it('returns 401 for requests without Bearer token', async () => {
      const server = createMCPServer({
        name: 'mcp-sharepoint-test',
        version: '1.0.0',
        port: 0,
        auth: { token: 'test-sharepoint-token' },
      });
