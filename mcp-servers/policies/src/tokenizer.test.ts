...
describe('tokenizer (moved regression suite — compilePolicy(defaultResolvedPolicy()))', () => {
  describe('createPIIContext', () => {
    ...
    it('creates context with custom values', () => {
      const context = createPIIContext(policy, { maxTokens: 500, ttlMs: 60000 });
  ...
  describe('tokenizePII', () => {
    ...
    beforeEach(() => {
      context = createPIIContext(policy);
    });
    ...
    it('respects maxTokens limit', () => {
      ...
      const limitedContext = createPIIContext(policy, { maxTokens: 2, ttlMs: 30000 });
  ...
  describe('cleanupExpiredTokens', () => {
    it('removes expired tokens', () => {
      ...
      const context = createPIIContext(policy, { maxTokens: 100, ttlMs: 1000 }); // 1 second TTL
    ...
    it('keeps non-expired tokens', () => {
      ...
      const context = createPIIContext(policy, { maxTokens: 100, ttlMs: 10000 }); // 10 second TTL
  ...
  describe('sensitive key token limit', () => {
    it('returns original value when token limit is reached for sensitive field', () => {
      ...
      const limitedContext = createPIIContext(policy, { maxTokens: 1, ttlMs: 30000 });
  ...
  describe('detokenize with token-like values', () => {
    ...
    it('leaves a valid-hex token unchanged when it is not in the context map', () => {
      // Token pattern matches [A-Z][A-Z0-9_]*:TOKEN_[A-F0-9]+ — covers the false branch of
      // `if (entry)` in detokenizeString
      const context = createPIIContext(policy);
