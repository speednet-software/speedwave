...
- **APIs**: Jira Cloud REST v3 + Agile 1.0; Confluence Cloud REST v2 (pages/comments/labels/attachments) + v1 (CQL search)
- **Scope**: Atlassian Cloud only in v1 (`*.atlassian.net`; no Data Center/Server)
...
src/
  ...
  adf.ts                    # textToAdf / storageBody helpers + scope-guard assertions
  types.ts                  # hand-written DTOs
  domains/                  # one module per resource (factory + normalisers)
