...
```
 ...
 ═══════════════════════════════════════════════════════════════
  ...
  │    ├─ macOS arm64    ─► .dmg + .app.tar.gz + .sig  (3)     │
  │    ├─ macOS x86_64   ─► .dmg + .app.tar.gz + .sig  (3)     │
  │    ├─ Linux x86_64   ─► .deb + .AppImage + .sig    (4)     │
...
Expected assets per release (~18):

- 13 Tauri desktop artifacts (3 per macOS target + 4 Linux + 3 Windows — includes `.sig` signatures; each Desktop bundle also includes the CLI binary in its resources)
