...
Expected assets per release (~17):

- 12 Tauri desktop artifacts (3 per macOS target + 3 Linux + 3 Windows — includes `.sig` signatures; each Desktop bundle also includes the CLI binary in its resources)
...
macOS artifacts use user-friendly architecture labels:

- `Speedwave_<version>_macOS_Apple_Silicon.dmg` / `.app.tar.gz` (ARM64, Apple Silicon Macs)
- `Speedwave_<version>_macOS_Intel.dmg` / `.app.tar.gz` (x86_64, Intel Macs)
...
The `publish-release` job explicitly verifies each required updater archive has a non-empty `.sig` partner, asserts `latest.json.platforms` contains the 7 required keys (`darwin-*`, `windows-*`), and enforces `tauri.conf.json` updater-plugin shape (`createUpdaterArtifacts`, `endpoints`, `pubkey`) and anti-drift between the 13 release-please-managed version sources via `_tests/desktop/updater-config.bats` at PR time.
