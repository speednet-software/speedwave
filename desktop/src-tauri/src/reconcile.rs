...
pub(crate) struct ExitCleanupContext {
    ...
    /// In-process PII NER detector service (ADR-089) — stopped + lock removed on exit.
...
pub(crate) fn reconcile_compose_port(app_handle: &tauri::AppHandle) {
    ...
    std::thread::spawn(move || {
        ...
        // Current host ports: mcp-os from its lock.json, the PII NER detector from its live lock.
        let data_dir = speedwave_runtime::consts::data_dir();
