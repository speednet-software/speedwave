//! OR of enabled integrations across all projects; used to decide if a worker
//! image is still needed somewhere before pruning it on disable.

use speedwave_runtime::config::{self, ResolvedIntegrationsConfig, SpeedwaveUserConfig};
use std::path::Path;

/// ORs every project's resolved integrations; missing/unreadable dirs are warned and skipped.
