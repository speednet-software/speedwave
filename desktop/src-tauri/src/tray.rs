...
//! bits; callers mutate via its accessors then call `refresh_tray_menu`. ADR-054.
...
mod tests {
    ...
    fn spec_setup_complete_with_update_keeps_install_and_beta() {
        // ADR-054 regression: toggling beta must not drop "Install Update".
    ...
    fn spec_setup_incomplete_hides_beta_even_with_update() {
        // ADR-054 regression: beta toggle must not appear before setup.
