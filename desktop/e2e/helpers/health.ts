...
export async function waitForHealthy(project: string): Promise<void> {
  ...
  try {
    await browser.waitUntil(
      async () => {
        ...
        return (
          ...
          result.containers.some((c) => c.name === 'claude') &&
          result.containers.some((c) => c.name === 'mcp_hub') &&
    ...
    throw new Error(
      `Containers for '${project}' not healthy within 120s. Last: ${lastObservation}`
    );
  }
}
