...
export interface ChatStoreDeps {
  ...
  anthropicModels: AnthropicModelsService;
  control: ClaudeControlService;
  planUsage: PlanUsageService;
  clipboard: Clipboard;
  log: LoggerService;
  /** Ensures the (service-owned, single per project) stream listener is set up. */
  ensureListeners: () => Promise<void>;
}

/** Per-tab chat session state and lifecycle; one instance per open tab. */
export class ChatSessionStore {
  /**
   * Builds a store bound to one chat tab.
   * @param tabId - Stable id sent on every session-scoped Tauri command.
   * @param deps - Injected services and service-owned callbacks.
   */
  constructor(
  ...
  async applyEffortSelection(level: string): Promise<void> {
    ...
    try {
      await this.deps.tauri.invoke('set_effort_pin', {
  ...
  private async sessionTakesWireEffort(project: string): Promise<boolean> {
    try {
      return (
        (await this.deps.tauri.invoke<boolean>('get_chat_takes_wire_effort', { project })) === true
      );
    } catch (e: unknown) {
      this.deps.log.warn(`[chat-state] get_chat_takes_wire_effort failed: ${String(e)}`);
      return false;
    }
  }
  ...
  private async outlastRestart(): Promise<boolean> {
    const project = this.deps.projectState.activeProject();
    let restart = this.deps.projectState.restartInFlight;
    while (restart) {
      await restart;
      restart = this.deps.projectState.restartInFlight;
    }
    return (
      project === this.deps.projectState.activeProject() &&
      this.deps.projectState.status() !== 'switching'
    );
  }

  private reportSelectionFailure(what: string, cause: unknown): void {
    const msg = cause instanceof Error ? cause.message : String(cause);
    this.deps.log.warn(`${what} failed: ${msg}`);
    this._modelSelectionError.set(msg);
  }

  private async rerenderContainersForModel(): Promise<boolean> {
    const outcome = await this.deps.projectState.restartContainers();
    if (outcome === 'restarted') return true;
    if (outcome === 'failed') {
      this.reportSelectionFailure(
        'compose re-render for the picked model',
        this.deps.projectState.restartError
      );
      this.deps.projectState.requestRestart();
      return false;
    }
  ...
  /** Per-tab composer draft text (phase 3 wires this into the composer). */
  ...
  /** Re-reads plan limits and context usage from the live session's control channel. */
  async refreshControlData(): Promise<void> {
    if (this._activeKind === null) await this.refreshLlmConfigCache();
    const project = this.deps.projectState.activeProject();
    if (!project) return;
    if (this._activeKind === 'anthropic_oauth') void this.deps.planUsage.refresh(project);
  ...
  private async recordRateLimit(info: RateLimitInfo): Promise<void> {
    ...
    if (project && this._activeKind === 'anthropic_oauth') {
      this.deps.planUsage.recordSignal(project, info);
    }
  }
  ...
  readonly newConversationBlockedReason: Signal<string> = computed(() => {
    ...
    if (!this.deps.projectState.activeProject()) return NEW_CONVERSATION_NO_PROJECT;
    if (this.deps.projectState.status() !== 'ready') return NEW_CONVERSATION_BUSY;
    return '';
  });
  ...
  async init(): Promise<void> {
    this.deps.log.debug(`[chat-state] init: initialized=${this.initialized}`);
    await this.deps.ensureListeners();
    if (!this.initialized) {
      this.initialized = true;
      if (this.deps.projectState.status() === 'ready') {
        void this.startChatSession();
      } else {
        const gen = this._sessionGeneration;
        const unsub = this.deps.projectState.onProjectReady(() => {
          unsub();
      ...
      try {
        ...
        if (gen !== this._sessionGeneration) {
          ...
          if (isNotAuthenticatedError(msg)) {
            ...
            this.deps.log.error(`[chat-state] Failed to start chat session: ${msg}`);
            this.deps.projectState.status.set('error');
            this.deps.projectState.error = `Failed to start chat session: ${msg}`;
            this.notifyChange();
          }
        }
      } finally {
  ...
  async sendMessage(input: string | ChatInput, displayText?: string): Promise<void> {
    ...
    this.deps.log.debug(`[chat-state] sendMessage: isStreaming=${this.isStreaming}`);
    ...
    const invokeArgs = { blocks: wireBlocks, displayText: surfaceText, tabId: this.tabId };
    try {
      await this.deps.ensureListeners();
  ...
  async submitAnswer(toolUseId: string, questionIdx: number, value: string): Promise<void> {
    ...
    try {
      await this.deps.tauri.invoke('submit_question_answer', {
        toolUseId,
        questionIdx,
        answer: value,
        tabId: this.tabId,
      ...
      if (capturedTurn !== this._turnId) {
        this.deps.log.debug(
          `[chat-state] submitAnswer: suppressing error after stop: ${String(err)}`
        );
        return;
      }
      this.isStreaming = false;
  ...
  async stopConversation(): Promise<void> {
    ...
    try {
      ...
      if (msg.includes('no active session')) {
        this.deps.log.debug(`[chat-state] stopConversation: backend already idle: ${msg}`);
        return;
      }
      this.deps.log.error(`[chat-state] stopConversation: invoke failed: ${msg}`);
      this._messages = [
        ...this._messages,
        {
          role: 'assistant',
  ...
  resetForNewConversation(): void {
    this.deps.log.debug('[chat-state] resetForNewConversation');
    this._sessionGeneration += 1;
  ...
  async queueMessage(text: string): Promise<string | null> {
    ...
    try {
      const displaced = await this.deps.tauri.invoke<{ text: string; queued_at: number } | null>(
        'queue_message',
        { sessionId, text }
      );
      return displaced?.text ?? prior;
    } catch (err) {
      this.deps.log.warn(`[chat-state] queueMessage: backend invoke failed: ${String(err)}`);
      return prior;
    }
  }
  ...
  private async flushDeferredQueue(sessionId: string): Promise<void> {
    ...
    if (!queued) return;
    try {
      await this.deps.tauri.invoke('queue_message', { sessionId, text: queued.text });
    } catch (err) {
      this.deps.log.warn(`[chat-state] flushDeferredQueue: backend invoke failed: ${String(err)}`);
    }
  }
  ...
  async cancelQueuedMessage(): Promise<void> {
    ...
    try {
      await this.deps.tauri.invoke('cancel_queued_message', { sessionId });
    } catch (err) {
      this.deps.log.warn(`[chat-state] cancelQueuedMessage: backend invoke failed: ${String(err)}`);
    }
    this._pendingQueue = null;
    this.notifyChange();
  }
  ...
  copyMessage(index: number): boolean {
    ...
    const ok = this.deps.clipboard.copy(text);
    if (!ok) {
      this.deps.log.warn('[chat-state] copyMessage: clipboard write failed');
    }
    return ok;
  }
  ...
  async retryLastAssistant(): Promise<void> {
    ...
    try {
      await this.deps.tauri.invoke('retry_last_turn', {
        sessionId,
        userUuid,
        tabId: this.tabId,
      });
  ...
  /** Resumes the durable session after a container restart, asking the decider when history may not fit. */
  async decideResumeAfterRestart(): Promise<void> {
  ...
  private async startFreshSession(): Promise<void> {
    try {
      ...
      this.deps.log.error(`[chat-state] startFreshSession failed: ${String(err)}`);
      this._messages = [
        ...this._messages,
  ...
  async resumeConversation(sessionId: string): Promise<void> {
    ...
    try {
      ...
      const transcriptPromise = this.deps.tauri
        .invoke<ConversationTranscript>('get_conversation', { project, sessionId })
        .catch((err) => {
          this.deps.log.error(`[chat-state] get_conversation failed: ${String(err)}`);
          return null;
        });
      const resumePromise = this.deps.tauri.invoke('resume_conversation', {
        project,
        sessionId,
        tabId: this.tabId,
      });
      ...
      this.deps.log.error(`[chat-state] resumeConversation failed: ${String(err)}`);
      const msg = String(err);
      if (isNotAuthenticatedError(msg)) {
        outcome = 'auth';
        await this.deps.projectState.retryAuth();
      } else {
        outcome = 'failed';
        this.loadMessages([
  ...
  private async reconcileFooterCost(assistantUuid: string | undefined, attempt = 0): Promise<void> {
    ...
    const capturedTurn = this._turnId;
    try {
      const u = await this.deps.tauri.invoke<ResponseUsage | null>('get_usage_for_response', {
        project,
      ...
      if (isTerminalCostSource(u.cost_source)) {
        ...
        if (cur) {
          ...
          const conversationCost = await this.deps.tauri.invoke<number | null>(
      ...
      const backoff = ChatSessionStore.DEFERRED_RECONCILE_BACKOFF_MS;
      ...
      this.deps.log.debug(`[chat-state] reconcileFooterCost best-effort failed: ${String(err)}`);
    }
  }
  ...
  async refreshLlmConfigCache(): Promise<void> {
    try {
      const config = await this.deps.tauri.invoke<LlmConfigResponse>('get_llm_config');
      ...
      if (project && this._activeKind === 'anthropic_oauth' && kind !== 'anthropic_oauth') {
        this.deps.planUsage.drop(project);
      }
      ...
      this.deps.log.debug(`[chat-state] refreshLlmConfigCache failed: ${String(err)}`);
    }
  }
