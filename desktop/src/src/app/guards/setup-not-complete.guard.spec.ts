...
import { setupNotCompleteGuard } from './setup-not-complete.guard';
import { TauriService } from '../services/tauri.service';
import { MockTauriService } from '../testing/mock-tauri.service';

describe('setupNotCompleteGuard', () => {
  ...
  it('should allow access when setup is not complete', async () => {
    ...
    const result = await TestBed.runInInjectionContext(() =>
      setupNotCompleteGuard({} as never, {} as never)
    );
    expect(result).toBe(true);
  });

  it('should redirect to /settings when setup is complete', async () => {
