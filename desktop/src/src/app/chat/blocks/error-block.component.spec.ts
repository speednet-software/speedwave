...
describe('ErrorBlockComponent', () => {
  ...
  it('happy: renders content and default generic shape', () => {
    ...
    expect(wrapper?.classList.contains('err-timeline')).toBe(true);
    expect(wrapper?.classList.contains('err-timeline-red')).toBe(true);
  });
  ...
  for (const kind of redKinds) {
    it(`happy: ${kind} renders a red timeline (no ring, no callout)`, () => {
      setInputs('oops', kind);
      const wrapper = el().querySelector('[data-testid="error-block"]');
      expect(wrapper?.classList.contains('err-timeline')).toBe(true);
      expect(wrapper?.classList.contains('err-timeline-red')).toBe(true);
      expect(wrapper?.classList.contains('err-callout')).toBe(false);
  ...
  for (const kind of amberKinds) {
    it(`happy: ${kind} renders an amber callout with an action button`, () => {
      setInputs('please do X', kind);
      const wrapper = el().querySelector('[data-testid="error-block"]');
      expect(wrapper?.classList.contains('err-callout')).toBe(true);
      expect(wrapper?.classList.contains('err-timeline')).toBe(false);
      expect(el().querySelector('[data-testid="error-action"]')).toBeTruthy();
    });
  }

  it('happy: stopped_by_user renders a gray timeline', () => {
    setInputs('turn cancelled', 'stopped_by_user');
    const wrapper = el().querySelector('[data-testid="error-block"]');
    expect(wrapper?.classList.contains('err-timeline-gray')).toBe(true);
    expect(wrapper?.classList.contains('err-timeline-red')).toBe(false);
  });
